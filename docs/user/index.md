...
UESF 在设计上着重解决 EEG 研究中常见的数据泄露问题：切分器支持按被试/会话/数据集维度隔离（同一被试的数据不会同时出现在训练集和测试集），在线变换严格执行 Fit-on-Train 原则（标准化参数只从训练集计算）。此外，框架原生支持 UDA（无监督域自适应）实验：跨数据集和数据集内两种 UDA 类型，holdout/k-fold 两种策略，transductive/inductive 两种变体均可通过 YAML 配置灵活组合，并提供跨数据集通道对齐和标签一致性验证。
