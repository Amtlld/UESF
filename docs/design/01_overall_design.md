...
- **全局配置**：采用"数据库存储默认值 + `~/.uesf/config.yml` 文件覆写"的双层机制，当前支持 `data_dir`、`default_device`、`num_workers`、`log_level` 四个配置项
- **SQLite 数据库**：统一存储在 `~/.uesf/uesf.db`，管理数据集、模型、训练器、评估指标、实验和配置的全部元信息。支持 Schema 版本迁移和事务保护
