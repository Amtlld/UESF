...
| 算子名称 (`name`) | 说明 | 核心参数 (`params`) |
| :--- | :--- | :--- |
| `sliding_window` | **滑窗切片 (Epoching)**。<br>将连续片段切割为供模型消费的标准 Batch。 | `window_size_sec`: 窗口截断长(秒)<br>`stride_sec`: 移动步长(秒)<br>`label_strategy`: "mode"/"last" |
