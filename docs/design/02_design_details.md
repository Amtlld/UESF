...
CREATE TABLE raw_datasets (
    ...
    label_mapping TEXT NOT NULL, -- 以JSON对象形式存储的数字标签与字符串语义标签的映射关系（如 {"0": "angry", "1": "happy"}）。必须由用户指明，取代旧的 categories 字段，作为标签类别的唯一定义来源
...
CREATE TABLE preprocessed_datasets (
    ...
    dimension_info TEXT NOT NULL, -- JSON对象，字符串列表，表示各维度对应的数据意义，如 ["subject", "record", "channel", "sample"]。必须由用户指明
    label_shape TEXT, -- 由系统在预处理完成后自动推断
    label_mapping TEXT NOT NULL, -- 以JSON对象形式存储的数字标签与字符串语义标签的映射关系（如 {"0": "angry", "1": "happy"}）。继承自源原始数据集或通过预处理管线中的标签处理模块重新定义
...
raw:
  ...
  label_mapping:             # (必填) 数字标签与语义标签的映射，取代旧的 n_classes 字段
...
class BaseModel(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        :param n_classes: 分类任务的类别数（由框架从数据集 label_mapping 自动推算注入）
...
training:
  ...
  batch_size: <int>
  learning_rate: <float>
  optimizer: <optimizer>          # 优化器名称，如 "adam", "sgd", "adamw" 等
...
- `uesf experiment resume --exp <experiment_name>`: 实验断点续训与异常中断恢复命令。如果一个实验遭遇中途 OOM 或是系统崩溃退出，该指令会检索 `experiments` 表内的状态与环境快照，锁定最近的 Checkpoint 权重文件，并完美还原 DataLoader 的进度以及优化器动量，实现深层无损恢复。
